  ...
  if (clickable) {
    return (
      <button
        ...
        className={`inline-flex items-center gap-1 ${px} rounded-full bg-transparent border border-[#C9A84C] text-[#C9A84C] ${text} font-medium hover:bg-[#C9A84C]/10 transition-colors cursor-pointer`}
      >
        ✦ True Potential
      </button>
    );
...
export function PortfolioCard({ company, onClick }) {
  return (
    <button
      ...
      className="relative w-full text-left bg-white border border-gray-200 rounded-xl p-5 transition-all duration-200 hover:border-[#6D4A9E] hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#6D4A9E]/10 group"
...
export function EmergingCard({ venture, onClick }) {
  return (
    <button
      ...
      className="w-full text-left bg-white border border-dashed border-gray-300 rounded-xl p-5 transition-all duration-200 hover:border-[#C9A84C]/50 hover:-translate-y-0.5 hover:shadow-lg hover:shadow-[#C9A84C]/5 group"
    >
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        <SectorPill sectorId={venture.sector} />
        <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-mono font-medium bg-[#C9A84C]/10 border border-[#C9A84C]/30 text-[#C9A84C]">
          Opportunity
        </span>
      </div>
    ...
    >
      <div className="flex items-center gap-2 mb-3 flex-wrap">
        ...
        {insight.insightPageSlug && (
          <span className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-mono text-[#6D4A9E] bg-[#6D4A9E]/10 border border-[#6D4A9E]/20">
            Full Report
          </span>
        )}
      </div>
